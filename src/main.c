...
void test_nops_psram_1(void) {
#include "bad_practice.inc"
}       /* END; */

#pragma section

//void __attribute__((section(".ram_code"))) test_nops_psram_2(void) {
//#include "bad_practice.inc"
//}

void test_nops_flash(void) {
#include "bad_practice.inc"
}
...
//		printf("%p\n", test_nops_psram_2);
		...
		uint32_t i=0;
		for(; i<(uint32_t)__PMI_PSPR_SIZE; i+=2){
			if(0==i%(16*2)) {
				printf("\n(%04X) %X_ ", i, (uint32_t)__PMI_PSPR_SIZE);
				flush_stdout();
			}
			printf("%04X ", *(uint16_t*)(((uint32_t)__PMI_PSPR_BEGIN)+i));
		}
		printf("\n \n\n");
		flush_stdout();
