...
//	}
	SRC_GPSR00.B.SRR = 0;
	InterruptInstall(SRC_ID_GPSR00, prvInterruptYield, configKERNEL_YIELD_PRIORITY, 0);
...
void prvSetupTimerInterrupt( void )
{
	...
	uint32_t reload_value = CMP0_MATCH_VAL;

	InterruptInstall(SRC_ID_STM0SR0, prvSystemTickHandler, configKERNEL_INTERRUPT_PRIORITY, reload_value);
...
static void prvSystemTickHandler( int iArg )
{
	...
	MODULE_STM0.CMP[0].U += (uint32_t)iArg;
